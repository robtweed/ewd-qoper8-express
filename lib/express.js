...
function handleMessage(req, res, next) {
  ...
  var handleResponse = function(resultObj) {
    ...
    if (resultObj.type === 'error' && typeof resultObj.message === 'string') {
      resultObj.message = {
        error: 'Unexpected Exception occurred: ' + resultObj.message
      };
    }
    ...
    if (message.error) {
      ...
      if (application) {
        if (typeof qoper8.workerResponseHandlers[application] === 'undefined') {
          ...
          try {
            var handlerModule = require(moduleName) || {};
            if (handlerModule.restModule && handlerModule.up) {
              // dynamic module needs initialising to build it!
              // signal that this is the master process initialising it via true 2nd argument
              handlerModule.init(application, true);
            }
            qoper8.workerResponseHandlers[application] = handlerModule.workerResponseHandlers;
          }
          catch(err) {
            console.log(err);
        ...
        if (type && qoper8.workerResponseHandlers && qoper8.workerResponseHandlers[application] && qoper8.workerResponseHandlers[application][type]) {
          message = qoper8.workerResponseHandlers[application][type].call(qoper8, message, req);
        }
        if (message) {
